...
def menu():
    ...
    while menu == True:
        if (
            float(r.get(str(name) + "_balance")) <= 0
            and float(r.get(str(name) + "_savings")) <= 0
        ):
        ...
        if choice == "1" or choice == "Help" or choice == "help":
            ...
            choice == "Admin"
            or choice == "admin"
            and r.get(str(name) + "_status") == "admin"
        ):
            # Admin Menu
    ...
    if r.exists(str(name) + "_pin"):
        ...
        while decide == False:
            ...
            if (
                ...
                clear()
                print(
                    "* Good choice and welcome to WAIO Bank,", str(name).title() + "!"
                )
                decide = True
            else:
...
def intialise():
    try:
        ...
        print("{:*^20}".format("ＷＡＩＯ Ｂａｎｋ ｖ１.０"), "\n")
        while True:
            name = input("* Enter Username to Log In: ").strip()
